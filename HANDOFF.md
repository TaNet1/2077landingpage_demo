...
### 2026-06-25 · Codex：新增「产品的能力」十大能力模块

参考：`D:/Downloads/runoob-test (9).html`
...
模块位置：`Why 2077.AI` 之后、原“三者协同 / Product Logic”之前，锚点为 `#capabilities`。
...
- 桌面端使用交错 Z 型布局，文字与对应动态演示左右交替。
- 移动端改为每项“文字 → 演示”的纵向顺序。
- 新模块使用现有浅色 Grainient 背景与品牌紫色，视觉语言与 Why 区域连续。
- 已补齐简体中文、繁体中文和英文文案。

动画优化共两轮：

1. 第一轮：使用 `IntersectionObserver` 控制 `is-visible / is-running`，动画只在对应能力进入视区后运行，离屏自动停止；文字和视觉使用不同延迟的分层进场。
2. 第二轮：为动画周期加入始终可读的基础状态，避免刚进入时出现空白；统一扫描、解析、锁定和完成阶段的节奏；桌面细指针设备加入轻量视差，移动端禁用。

验证：

- 10 项能力 DOM 均存在，当前视口只运行 1 项能力动画，离屏项动画为 `none`。
- 1280px 桌面中英文均无模块横向溢出。
- 390px 移动端 10 项均为单列，模块自身无横向溢出。
